...
async function getProvider(tenantId:number, providerName:string){
  return knex('tenant_oauth_providers').where({ tenant_id: tenantId, provider_name: providerName }).first();
}

// Redirect to provider
router.get('/:provider', async (req:any, res) => {
...
router.get('/:provider/callback', async (req:any, res) => {
  ...
  try {
    ...
    if (!profile || !profile.email) return res.status(400).send('no_email');
    // find or create user
    let user = await knex('users').where({ email: profile.email, tenant_id: tenant.id }).first();
    if (!user){
      const pwd = Math.random().toString(36).slice(2);
      const pwdHash = await hashPassword(pwd);
      const [id] = await knex('users').insert({ tenant_id: tenant.id, email: profile.email, password_hash: pwdHash, role: 'user', created_at: new Date() }).returning('id');
  ...
  } catch (err:any){ console.error('oauth callback error', err); res.status(500).json({ error: 'oauth_failed', message: err && err.message }); }
});
