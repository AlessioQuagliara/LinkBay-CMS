...
router.get('/', async (req, res) => {
  const list = await knex('public_integrations').where({ is_active: true }).select('*');
  res.json(list);
});

// install for tenant: saves config into tenant_integrations
router.post('/install/:id', async (req:any, res) => {
  const tid = Number(req.params.id);
  const tenant = req.tenant;
  if (!tenant) return res.status(400).json({ error: 'tenant_required' });
  const integration = await knex('public_integrations').where({ id: tid }).first();
  if (!integration) return res.status(404).json({ error: 'not_found' });
  const cfg = req.body.config || {};
  const [id] = await knex('tenant_integrations').insert({ tenant_id: tenant.id, provider: integration.name.toLowerCase(), config: JSON.stringify(cfg), is_active: true, created_at: new Date() }).returning('id');
...
router.get('/configure/:id', async (req:any, res) => {
  const id = Number(req.params.id);
  const integration = await knex('public_integrations').where({ id }).first();
