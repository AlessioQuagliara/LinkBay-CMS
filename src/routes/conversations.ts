...
router.post('/', async (req, res) => {
  ...
  const tenantDb = getTenantDB(tenant.id);
  const inserted: any = await tenantDb('conversations').insert({ subject, status: 'open' }).returning('*');
  ...
  for (const p of parts) {
    if (p) notifyUser(p, 'conversation.created', { conversation: convo, tenant_id: tenant.id });
  }
...
router.post('/:id/messages', async (req, res) => {
  ...
  const convId = Number(req.params.id);
  const { body } = req.body;
  const inserted: any = await tenantDb('messages').insert({ conversation_id: convId, user_id: user.id, body }).returning('*');
  ...
  participants.forEach((p:any)=>{ if (p && p.user_id) notifyUser(p.user_id, 'conversation.message', { message: msg, tenant_id: tenant.id }); });
