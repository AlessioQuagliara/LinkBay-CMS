...
router.post('/login-redirect', async (req, res) => {
  ...
  if (!tenantParam && !emailParam) return res.redirect('/login');
  ...
  try {
    if (!tenantSubdomain && emailParam) {
      ...
      if (user) {
        ...
        if (t) tenantSubdomain = t.subdomain;
      }
    }
  } catch (e) {
  ...
  if (!tenantSubdomain) return res.status(404).json({ error: 'tenant_not_found' });
