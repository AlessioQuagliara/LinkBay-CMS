...
<head>
    ...
    <title>Accetta invito — {{ $agencyName }}</title>
...
<div class="w-full max-w-md">
    <div class="text-center mb-8">
        ...
        <h1 class="text-2xl font-bold text-[#343a4D]">Unisciti al team</h1>
        <p class="text-gray-500 mt-1 text-sm">
            Sei stato invitato a far parte di
            <span class="font-semibold text-[#343a4D]">{{ $agencyName }}</span>
            come <span class="font-semibold">{{ $member->roleLabel() }}</span>.
        </p>
    </div>
    ...
    <div class="bg-white rounded-2xl shadow-sm border border-gray-200 p-8">
        ...
        <form method="POST" action="{{ route('agency-invite.accept', ['token' => $token]) }}" class="space-y-5">
            @csrf

            <div>
                <label class="block text-sm font-medium text-gray-700 mb-1">Email</label>
                <input type="text"
                    value="{{ $member->invited_email }}"
                    disabled
            ...
            <div>
                ...
                <input
                    ...
                    value="{{ old('name') }}"
                    autocomplete="name"
                    class="w-full px-3.5 py-2.5 rounded-xl border border-gray-200 focus:border-[#ff5758] focus:ring-2 focus:ring-[#ff5758]/20 text-sm outline-none transition @error('name') border-red-400 @enderror"
