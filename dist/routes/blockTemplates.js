...
router.get('/', async (req, res) => {
    ...
    const q = (0, db_1.knex)('block_templates').select('id', 'tenant_id', 'name', 'preview_image_url', 'category', 'is_public', 'created_at');
    if (isPublic)
        q.where('is_public', true).whereNull('tenant_id');
    else if (tenant && tenant.id)
        q.where(function () { this.where('tenant_id', tenant.id).orWhere('is_public', true).orWhereNull('tenant_id'); });
...
router.post('/', async (req, res) => {
    ...
    const [id] = await (0, db_1.knex)('block_templates').insert(insert).returning('id');
    res.json({ id });
});
